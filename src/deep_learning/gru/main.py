...
class GRUPredictor(nn.Module):
    """GRU-based sequence predictor for vessel trajectories.

    Lighter than LSTM (no cell state) while retaining strong sequence
    ...
    dropout : float
        Dropout between GRU layers (ignored when num_layers=1). Default ``0.0``.
    """
...
if __name__ == "__main__":
    ...
    model     = GRUPredictor(input_size=FEATURES, hidden_size=64, num_layers=2, output_size=1)
